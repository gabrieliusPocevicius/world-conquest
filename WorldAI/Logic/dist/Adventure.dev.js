...
var Settlement = function Settlement(name, pop, strength, walled, troops, food, wealth) {
  _classCallCheck(this, Settlement);

  //city name
  this.name = name; // city population 

  this.pop = pop; // total city stength

  this.stength = strength; //is the city walled  height of the wall

  this.walled = walled; // garrison troops 

  this.troops = troops; // food the city stores

  this.food = food; // total wealth of the settlement

  this.wealth = wealth;
  this.wallWidth = wallWidth;
  walled = false; // settlement sizes
  //not tested

  var settlement = ["SMALL VILLAGE", "VILLAGE", "SMALL TOWN", "TOWN", "SMALL CITY", "CITY", "CAPITAL CITY", "MEGA METROPOLIS"]; // types of sizes
  // not tested
  ...
  function market() {
    ...
    console.log("trade".concat(trade));
    return trade;
  }

  function farm() {
    var fertility = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : Math.random() * 100;
    var trade = arguments.length > 1 ? arguments[1] : undefined;
  ...
  function enter() {
    ...
    function settlementSize() {
      ...
      if (this.pop > 50000 && this.pop <= 100000) {
